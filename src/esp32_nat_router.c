...
#define DISP_POWER_CTRL_GPIO 16
...
//Initializing the ADC pin and making it as an output
void init_adc_sense_gpio(void) {
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << ADC_SENSE_EN_GPIO),
...
void configure_vbus_sense()
{
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << VBUS_SENSE_PIN),  // Configure the VBUS_SENSE_PIN
        .mode = GPIO_MODE_INPUT,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,  // Enable pulldown since default is 0
...
int get_wifi_signal_strength() {
    ...
    return -100;  // Return a low value if not connected
}
...
void getStringFromNVS()
{
    ...
    if (ap_ssid == NULL)
    {
        ap_ssid = param_set_default("Destra Router");
    }
...
void app_main(void)
{
    ...
    // Set power latch high immediately to keep the board powered
    gpio_set_level(POWER_LATCH_GPIO, 1);
    
    // Wait for the initial button release
    while (gpio_get_level(DISP_BUTTON_GPIO) == 0) {
        vTaskDelay(pdMS_TO_TICKS(10));
    }

    // Create button monitoring task
    xTaskCreate(button_monitoring_task, "button_monitor", 4096, NULL, 5, NULL);
    // button_init(DISP_BUTTON_GPIO, POWER_LATCH_GPIO, DISP_POWER_CTRL_GPIO);
    ...
    // Initialize the ADC sense pin
    init_adc_sense_gpio();

    // Initialize the VBUS sense pin
    configure_vbus_sense();
 ...
 /*   float BatValue=0;
    ...
    //new_value= adc_raw;
    BatValue =((adc_raw*3.3)/4095)*2.1;
    //avarage+=adc_data;
    vTaskDelay(pdMS_TO_TICKS(10));  // Small delay for stabilization
    gpio_set_level(ADC_SENSE_EN_GPIO, 0);
    if(BatValue<3.4 && vbus_state !=1)
    {  
        ESP_LOGI(TAG, "adc_raw %d, vlotage %.2f ", adc_raw, BatValue);
